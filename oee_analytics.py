...
def open_productions(events: pd.DataFrame, as_of: datetime | None = None) -> pd.DataFrame:
    ...
    chiusura. Lo stato di attenzione dipende dall'ultimo segmento salvato.
    """
    columns = [
    ...
    for (machine, lot), current in active.items():
        ...
        age_hours = max((now - last_at).total_seconds() / 3600, 0) if last_at != pd.Timestamp.min else 999999
        if last_at.date() == now.date():
            status = "🟢 IN LAVORAZIONE"
        elif age_hours <= 48:
            status = "🟡 DA RIPRENDERE"
        else:
...
def comber_planning_view(plans: pd.DataFrame, events: pd.DataFrame, selected_week_plans: pd.DataFrame, as_of=None):
    """Costruisce piano corrente, backlog FIFO e produzioni fuori piano."""
    ...
    before, _, _ = _allocate_comber(plans, events, week_start - timedelta(days=1))
    current, allocations, unmatched = _allocate_comber(plans, events, cutoff)
    previous = {str(r["piano_id"]): float(r["_allocated"]) for _, r in before.iterrows()}
    rows = []
    for _, row in current.iterrows():
        plan_id = str(row["piano_id"])
        planned = float(row["_planned"])
        allocated = float(row["_allocated"])
        allocated_before = previous.get(plan_id, 0.0)
        residual_before = max(planned - allocated_before, 0.0)
        residual = max(planned - allocated, 0.0)
        recovered = max(residual_before - residual, 0.0)
        start = row["_start"]
        end = row["_end"]
        if as_of.date() < start:
            expected = 0.0
        ...
        rows.append({
            **{column: row[column] for column in plans.columns},
            ...
            "kg_residui": residual,
            "kg_residui_inizio_settimana": residual_before,
            "kg_recuperati_settimana": recovered,
    ...
    selected_ids = set(selected_week_plans["piano_id"].astype(str))
    current_rows = status[status["piano_id"].astype(str).isin(selected_ids)].copy()
    backlog_rows = status[
        (pd.to_datetime(status["data_inizio"], errors="coerce").dt.date < week_start)
        & (status["kg_residui_inizio_settimana"] > 1e-6)
    ].copy()
    if not unmatched.empty:
        unmatched = unmatched[(unmatched["data_turno"] >= week_start) & (unmatched["data_turno"] <= cutoff)]
    return current_rows, backlog_rows, unmatched
