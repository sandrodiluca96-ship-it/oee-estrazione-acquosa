...
def open_productions(events: pd.DataFrame, as_of: datetime | None = None) -> pd.DataFrame:
    ...
    for column in ["macchina", "tipo_evento", "lotto", "tipo_produzione", "stato_lotto"]:
        if column not in work:
            work[column] = ""
    machine_lookup = {normalize_text(machine): machine for machine in PROCESS_MACHINES}
    work["_machine"] = work["macchina"].map(lambda value: machine_lookup.get(normalize_text(value), ""))
    production_stage = work["tipo_produzione"].map(normalize_text).isin({
        "APERTURA LOTTO", "PROSECUZIONE LOTTO", "CHIUSURA LOTTO", "SCARICO ESTRATTORE",
    })
    work = work[
        work["_machine"].ne("")
        & (work["tipo_evento"].map(normalize_text).eq("PRODUZIONE") | production_stage)
    ...
    for row in work.to_dict("records"):
        machine, lot = row.get("_machine", ""), str(row.get("lotto", "")).strip()
        key = (machine, normalize_text(lot))
        stage = normalize_text(row.get("tipo_produzione", ""))
        closed = stage == "CHIUSURA LOTTO" or normalize_text(row.get("stato_lotto", "")) == "COMPLETATO"
        if closed:
            active.pop(key, None)
            continue
    ...
    for (machine, lot), current in active.items():
        ...
        rows.append({
            "Macchina": machine, "Stato": status, "Lotto": latest_text("lotto") or lot,
...
def ordered_open_lots(events: pd.DataFrame, machine: str) -> list[str]:
    """Elenca i lotti non chiusi, dal più recentemente lavorato al più vecchio.

    La funzione serve al modulo di inserimento: quando esistono vecchi lotti
    rimasti formalmente aperti, il lotto appena lavorato deve essere la scelta
    predefinita per una prosecuzione.
    """
    if events.empty:
        return []
    work = events.copy()
    for column in ["macchina", "tipo_evento", "tipo_produzione", "lotto", "stato_lotto"]:
        if column not in work:
            work[column] = ""
    wanted_machine = normalize_text(machine)
    stages = work["tipo_produzione"].map(normalize_text)
    work = work[
        work["macchina"].map(normalize_text).eq(wanted_machine)
        & (
            work["tipo_evento"].map(normalize_text).eq("PRODUZIONE")
            | stages.isin({"APERTURA LOTTO", "PROSECUZIONE LOTTO", "CHIUSURA LOTTO", "SCARICO ESTRATTORE"})
        )
        & work["lotto"].astype(str).str.strip().ne("")
    ].copy()
    if work.empty:
        return []
    work["_timestamp"] = work.apply(_event_timestamp, axis=1)
    sort_columns = ["_timestamp"] + (["id_evento"] if "id_evento" in work else [])
    work = work.sort_values(sort_columns)
    active: dict[str, tuple[str, pd.Timestamp, int]] = {}
    for sequence, row in enumerate(work.to_dict("records")):
        lot = str(row.get("lotto", "")).strip()
        key = normalize_text(lot)
        closed = (
            normalize_text(row.get("tipo_produzione", "")) == "CHIUSURA LOTTO"
            or normalize_text(row.get("stato_lotto", "")) == "COMPLETATO"
        )
        if closed:
            active.pop(key, None)
        else:
            active[key] = (lot, row.get("_timestamp", pd.Timestamp.min), sequence)
    return [value[0] for value in sorted(active.values(), key=lambda value: (value[1], value[2]), reverse=True)]


def latest_machine_productions(events: pd.DataFrame) -> pd.DataFrame:
    """Restituisce una sola lavorazione: l'ultima salvata per ogni macchina."""
    columns = [
        "Macchina", "Stato", "Lotto", "Codice", "Descrizione", "Fase",
        "Ultimo aggiornamento", "Turno",
    ]
    if events.empty:
        return pd.DataFrame(columns=columns)
    work = events.copy()
    ...
    work = work.sort_values(sort_columns)
    rows = []
    for machine, machine_events in work.groupby("_machine", sort=False):
        last = machine_events.iloc[-1]
        lot_key = normalize_text(last.get("lotto", ""))
        history = machine_events[machine_events["lotto"].map(normalize_text) == lot_key]

        def latest_text(field):
            if field not in history:
                return ""
            for value in reversed(history[field].tolist()):
        ...
        closed = (
            normalize_text(last.get("tipo_produzione", "")) == "CHIUSURA LOTTO"
            or normalize_text(last.get("stato_lotto", "")) == "COMPLETATO"
        )
        status = "🔵 ULTIMA REGISTRATA" if closed else "🟢 IN CORSO"
        timestamp = last["_timestamp"]
        rows.append({
            "Macchina": machine,
            "Stato": status,
            "Lotto": latest_text("lotto"),
            "Codice": latest_text("codice"),
            "Descrizione": latest_text("descrizione"),
            "Fase": latest_text("fase_lavorazione") or latest_text("tipo_produzione"),
            "Ultimo aggiornamento": "—" if timestamp == pd.Timestamp.min else timestamp.strftime("%d/%m/%Y %H:%M"),
            "Turno": latest_text("turno"),
        })
...
def _allocate_comber(plans: pd.DataFrame, events: pd.DataFrame, cutoff: date, allocation_start: date | None = None):
    ...
    if allocation_start:
        extractions = extractions[extractions["data"] >= allocation_start]
    allocation_rows = []
    unmatched_rows = []
    ...
    for extraction in extractions.itertuples(index=False):
        ...
        for idx in order:
            if remaining <= 1e-9:
                break
            start = allocation_start or work.at[idx, "_start"]
...
def comber_planning_view(plans: pd.DataFrame, events: pd.DataFrame, selected_week_plans: pd.DataFrame, as_of=None):
    ...
    first_planned_day = pd.to_datetime(selected_week_plans["data_inizio"], errors="coerce").min().date()
    week_start = first_planned_day - timedelta(days=first_planned_day.weekday())
    ...
    current, allocations, unmatched = _allocate_comber(
        selected_week_plans, events, cutoff, allocation_start=week_start
    )
    # Lo stato del piano deve seguire la lavorazione live della macchina, non
    # tutti i vecchi lotti formalmente aperti per una chiusura dimenticata.
    active_comber = latest_machine_productions(events)
    active_comber = active_comber[
        (active_comber["Macchina"] == "Comber")
        & (active_comber["Stato"] == "🟢 IN CORSO")
    ]
    rows = []
    for _, row in current.iterrows():
        ...
        if active_lots and residual > 1e-6:
            status, light = "IN LAVORAZIONE", "🟢"
        rows.append({
