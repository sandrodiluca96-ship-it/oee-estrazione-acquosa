...
def render_oee_unified(turns, prod):
    ...
    for col,r in zip(cols,results):
        with col:
            ...
            c.metric("Qualità",f'{r["Qualità"]:.1%}')
...
area=st.sidebar.radio("Area",[
    "Dashboard unica","Comber","EV200","Spray Dryer",
...
if page=="Dashboard unica":
    st.title("Dashboard unica di produzione e OEE")
    st.markdown("### OEE delle tre macchine")
    all_turns=read_csv(TURNI_FILE,COL_TURNI); all_prod=prep_production()
    render_oee_unified(all_turns,all_prod)
    st.divider()
    st.markdown("### Production vs. Target — Estrazione ed essiccazione")
    ...
    df=all_prod; yesterday=today-timedelta(days=1); prev_end=month_start-timedelta(days=1); prev_start=prev_end.replace(day=1)
