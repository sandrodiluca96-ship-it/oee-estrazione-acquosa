...
from event_workflow import COL_EVENTI, EVENTI_FILE, render_machine_workflow
...
VERSIONE = "2.7.0"
QUALITA = 0.95
MACCHINE = ["Comber", "EV200", "Spray Dryer"]
DEFAULT_TARGET = {
...
.machine-head.comber{background:#0B4F87}.machine-head.ev200{background:#2F73B5}.machine-head.spray{background:#238B73}
...
def render_oee_unified(turns, prod):
    ...
    for col,r in zip(cols,results):
        with col:
            ...
            a.metric("Disponibilità",f'{r["Disponibilità"]:.1%}')
            b.metric("Performance",f'{r["Performance"]:.1%}')
            if r["Macchina"]=="Comber": metric_cols[2].metric("Mass yield media",f"{indicator:.1f}%")
            elif r["Macchina"]=="Spray Dryer": metric_cols[2].metric("Taglio medio",f"{indicator:.1f}%")
    ...
    st.metric("OEE complessivo ponderato",f"{overall:.1%}")
    if turns.empty:
...
if page=="Dashboard OEE":
    st.title("Dashboard OEE")
    st.caption("Cruscotto unico di Comber, EV200 e Spray Dryer. La qualità standard del 95% resta applicata internamente al calcolo OEE.")
    ...
    st.caption("Produzione fisica ed equivalente di estrazione Comber ed essiccazione Spray Dryer.")
    wide_report=st.toggle("Ingrandisci report",value=False,help="Nasconde il menu laterale e usa tutta la larghezza dello schermo. Disattiva il comando per tornare alla vista normale.")
    if wide_report:
        st.markdown("""
        <style>
    ...
    summary_band("Comber – Estrazione acquosa","navy",df,"Comber","equivalente",periods)
    summary_band("Spray Dryer – Essiccazione","green",df,"Spray Dryer","equivalente",periods)
    st.markdown(dashboard_table(df,today),unsafe_allow_html=True)
    cfg=target_config()
    target_txt=" · ".join(
        f"{r.macchina}: {r.target_fisico_giorno:g} kg fisici/g, {r.target_equivalente_giorno:g} kg eq./g, {r.giorni_produttivi_anno:g} gg/anno"
        for r in cfg.itertuples()
    )
    st.caption("Target attivi — "+target_txt)

elif page in ["Turno Comber", "Turno EV200", "Turno Spray Dryer"]:
    machine={"Turno Comber":"Comber", "Turno EV200":"EV200", "Turno Spray Dryer":"Spray Dryer"}[page]
    ...
    with tab_events:
        events=read_csv(EVENTI_FILE,COL_EVENTI)
        if machine_filter!="Tutte": events=events[events["macchina"]==machine_filter]
        if text_filter.strip() and not events.empty:
            needle=text_filter.strip().lower()
            emask=events[["lotto","codice","descrizione","tipo_evento"]].astype(str).apply(lambda c:c.str.lower().str.contains(needle,regex=False)).any(axis=1)
            events=events[emask]
    ...
    st.markdown(f'<div class="hint">Questo backup è indipendente: caricandolo vengono sostituiti soltanto i dati {machine}. Comber, EV200 e Spray Dryer non si sovrascrivono tra loro.</div>',unsafe_allow_html=True)
