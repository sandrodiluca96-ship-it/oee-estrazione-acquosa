...
from oee_analytics import (
    calculate_effectiveness,
    cause_id_for_name,
    comber_planning_view,
    normalize_text,
    open_productions,
)

# RELEASE OEE LAURIA 4.0.1 — produzioni aperte nella dashboard OEE/OOE
...
COL_CAUSALI = ["cause_id", "causale", "categoria", "esclusa_pianificato", "penalizza_ooe", "perdita_tecnica", "nota_obbligatoria", "attiva"]
...
def init_data():
    ...
    if not CAUSALI_FILE.exists():
        defaults = [
            ("Attesa prodotto","Organizzativa","SI","SI","NO","NO"),
            ("Attesa analisi","Organizzativa/Qualità","SI","SI","NO","NO"),
            ("Cambio lotto","Fermo programmato","SI","SI","NO","NO"),
            ("Inventario","Organizzativa programmata","SI","SI","NO","NO"),
            ("Lavaggio","Fermo programmato","SI","SI","NO","NO"),
            ("Pulizia","Fermo programmato","SI","SI","NO","NO"),
            ("Manutenzione programmata","Fermo programmato","SI","SI","NO","NO"),
            ("Guasto","Tecnica","NO","SI","SI","SI"),
            ("Manutenzione straordinaria","Tecnica","NO","SI","SI","SI"),
            ("Altro fermo non programmato","Operativa","NO","SI","NO","SI"),
        ]
        pd.DataFrame([
            {"cause_id":cause_id_for_name(name),"causale":name,"categoria":category,
             "esclusa_pianificato":excluded,"penalizza_ooe":ooe,"perdita_tecnica":technical,
             "nota_obbligatoria":note,"attiva":"SI"}
            for name,category,excluded,ooe,technical,note in defaults
...
def cause_config(persist_legacy=False):
    """Completa l'anagrafica v3 senza perdere causali o collegamenti storici."""
    causes=read_csv(CAUSALI_FILE,COL_CAUSALI).copy()
    legacy=causes["penalizza_ooe"].astype(str).str.strip().eq("")
    policy={
        "ATTESA PRODOTTO": ("Organizzativa","SI","NO","NO"),
        "ATTESA ANALISI": ("Organizzativa/Qualità","SI","NO","NO"),
        "CAMBIO LOTTO": ("Fermo programmato","SI","NO","NO"),
        "INVENTARIO": ("Organizzativa programmata","SI","NO","NO"),
        "LAVAGGIO": ("Fermo programmato","SI","NO","NO"),
        "PULIZIA": ("Fermo programmato","SI","NO","NO"),
        "MANUTENZIONE PROGRAMMATA": ("Fermo programmato","SI","NO","NO"),
        "GUASTO": ("Tecnica","NO","SI","SI"),
        "MANUTENZIONE STRAORDINARIA": ("Tecnica","NO","SI","SI"),
        "ALTRO": ("Operativa","NO","NO","SI"),
        "ALTRO FERMO NON PROGRAMMATO": ("Operativa","NO","NO","SI"),
    }
    changed=False
    for idx,row in causes.iterrows():
        if not str(row.get("cause_id","")).strip():
            causes.at[idx,"cause_id"]=cause_id_for_name(row["causale"]); changed=True
        if legacy.loc[idx]:
            key=normalize_text(row["causale"])
            category,excluded,technical,note=policy.get(
                key,(str(row.get("categoria","") or "Operativa"),str(row.get("esclusa_pianificato","") or "NO"),str(row.get("perdita_tecnica","") or "NO"),"SI")
            )
            causes.at[idx,"categoria"]=category
            causes.at[idx,"esclusa_pianificato"]=excluded
            causes.at[idx,"penalizza_ooe"]="SI"
            causes.at[idx,"perdita_tecnica"]=technical
            causes.at[idx,"nota_obbligatoria"]=note
            changed=True
    if persist_legacy and changed:
        save_csv(causes,CAUSALI_FILE,COL_CAUSALI)
    return causes


def save_cause_changes(original, edited):
    """Preserva il legame storico assegnando cause_id prima di rinominare."""
    old=original.copy(); new=edited.copy()
    for frame in [old,new]:
        for idx,row in frame.iterrows():
            if not str(row.get("cause_id","")).strip():
                frame.at[idx,"cause_id"]=cause_id_for_name(row.get("causale",""))
    old_by_id={str(r.cause_id):str(r.causale) for r in old.itertuples() if str(r.cause_id).strip()}
    events=read_csv(EVENTI_FILE,COL_EVENTI)
    linked_count=0
    for cause_id,old_name in old_by_id.items():
        mask=(events["cause_id"].astype(str).str.strip()=="")&(events["tipo_evento"].astype(str)==old_name)
        if mask.any():
            linked_count+=int(mask.sum())
            events.loc[mask,"cause_id"]=cause_id
    if linked_count:
        save_csv(events,EVENTI_FILE,COL_EVENTI)
    new_by_id={str(r.cause_id):str(r.causale) for r in new.itertuples() if str(r.cause_id).strip()}
    obsolete_names=[old_name for cause_id,old_name in old_by_id.items() if new_by_id.get(cause_id)!=old_name]
    if obsolete_names:
        soft_delete_ids(CAUSALI_FILE,obsolete_names)
    save_csv(new,CAUSALI_FILE,COL_CAUSALI)
    return linked_count, int(events["cause_id"].astype(str).str.strip().ne("").sum())
...
def _colonna_excel(frame,*aliases):
    lookup={_normalizza_intestazione(c):c for c in frame.columns}
    for alias in aliases:
        key=_normalizza_intestazione(alias)
        if key in lookup: return lookup[key]
    raise ValueError(f"Colonna non trovata: {aliases[0]}.")


def _numeri_excel(series):
    return pd.to_numeric(
        series.astype(str).str.strip().str.replace(".","",regex=False).where(
            series.astype(str).str.contains(",",regex=False),
...
def _leggi_esplosione_commessa(upload):
    sample=pd.read_excel(upload,header=None,nrows=8,dtype=str).fillna("")
    header_row=None
    for idx,row in sample.iterrows():
        labels={_normalizza_intestazione(v) for v in row}
        if "COMMESSA" in labels and "LOTTOPRODOTTO" in labels and "CODICECOMPONENTE" in labels:
            header_row=idx; break
    if header_row is None:
        raise ValueError("Nel file Esplosione commessa non è stata trovata la riga delle intestazioni.")
    return pd.read_excel(upload,header=header_row,dtype=str).fillna("")


def parse_historical_sources(carico_upload,explosion_upload):
    """Ricostruisce Spray Dryer da ATOM e Comber dai componenti MDR."""
    carico=pd.read_excel(carico_upload,dtype=str).fillna("")
    c_work=_colonna_excel(carico,"Lavorazione")
    c_date=_colonna_excel(carico,"Data di carico Prodiuzione","Data di carico Produzione")
    c_qty=_colonna_excel(carico,"Quantità caricata","Quantito caricata")
    c_lot=_colonna_excel(carico,"Codice Lotto")
    c_article=_colonna_excel(carico,"Articolo")
    c_desc=_colonna_excel(carico,"Descrizione")
    c_serial=_colonna_excel(carico,"Seriale")

    explosion=_leggi_esplosione_commessa(explosion_upload)
    e_order=_colonna_excel(explosion,"Commessa")
    e_row=_colonna_excel(explosion,"Riga")
    e_date=_colonna_excel(explosion,"Data Commessa")
    e_article=_colonna_excel(explosion,"Articolo Prodotto")
    e_desc=_colonna_excel(explosion,"Descrizione")
    e_output=_colonna_excel(explosion,"Quantità Prodotta")
    e_lot=_colonna_excel(explosion,"Lotto Prodotto")
    e_component=_colonna_excel(explosion,"Codice Componente")
    e_component_desc=_colonna_excel(explosion,"Descrizione.1")
    e_component_lot=_colonna_excel(explosion,"Lotto Componente")
    e_component_qty=_colonna_excel(explosion,"Quantità Scaricata")

    explosion["_lot"]=explosion[e_lot].astype(str).str.strip()
    explosion["_component"]=explosion[e_component].astype(str).str.strip()
    explosion["_component_desc"]=explosion[e_component_desc].astype(str).str.strip()
    explosion["_component_qty"]=_numeri_excel(explosion[e_component_qty])
    explosion["_output_qty"]=_numeri_excel(explosion[e_output])
    explosion["_date"]=pd.to_datetime(explosion[e_date],errors="coerce")
    history_cutoff=explosion["_date"].max()
    if pd.isna(history_cutoff):
        raise ValueError("Nel file Esplosione commessa non sono presenti date valide.")
    explosion["_is_malto"]=explosion["_component_desc"].str.upper().str.contains("MALTODESTRINA",regex=False)
    explosion["_is_mdr"]=explosion["_component"].str.upper().str.startswith("MDR")
    malto_by_lot=explosion[explosion["_is_malto"]].groupby("_lot")["_component_qty"].sum()

    atom=carico[carico[c_work].astype(str).str.strip().str.upper()=="ATOM"].copy()
    atom["_date"]=pd.to_datetime(atom[c_date],errors="coerce")
    atom["_qty"]=_numeri_excel(atom[c_qty])
    atom["_lot"]=atom[c_lot].astype(str).str.strip()
    atom=atom[
        atom["_date"].notna()&(atom["_date"]<=history_cutoff)&
        (atom["_qty"]>0)&atom["_lot"].ne("")
    ].copy()
    rows=[]; taglio_reale=0
    for r in atom.itertuples(index=False):
        data=r[atom.columns.get_loc("_date")]
        qty=float(r[atom.columns.get_loc("_qty")])
        lot=str(r[atom.columns.get_loc("_lot")])
        malto=float(malto_by_lot.get(lot,0))
        if malto>0 and malto<=qty:
            pure=qty-malto; taglio_reale+=1; taglio_source="da esplosione commessa"
        else:
            pure=qty*0.40; taglio_source="standard 40% per assenza distinta completa"
        serial=str(r[atom.columns.get_loc(c_serial)]).strip()
        code=str(r[atom.columns.get_loc(c_article)]).strip()
        description=str(r[atom.columns.get_loc(c_desc)]).strip()
        identity=serial or data.strftime("%Y%m%d")
        rows.append({
            "id":f"HIST-ATOM-{identity}-{lot}",
            "id_turno":f"STORICO-{data:%Y%m%d}-SPRAY","data_turno":data.date().isoformat(),
            "turno":"","macchina":"Spray Dryer","lotto":lot,"codice_droga":"",
            "codice_semilavorato":code,"descrizione":description,"kg_droga":0,"kg_liquido":0,
            "residuo_secco_pct":0,"kg_puro":pure,"kg_semilavorato":qty,
            "pct_puro_semilavorato":pct(pure,qty),"kg_puro_equivalente":max(pure,qty*0.40),
            "lotti_comber":"","piano_id":"",
            "note":f"STORICO_GESTIONALE_ATOM | Seriale {serial} | Taglio {taglio_source}",
        })

    group_cols=[e_order,e_row,e_date,e_article,e_desc,e_lot]
    comber_count=0
    for keys,group in explosion.groupby(group_cols,dropna=False,sort=False):
        order,row_no,date_value,article,description,lot=keys
        article=str(article).strip(); lot=str(lot).strip()
        if not article.upper().startswith(("WSD","YSD")) or not group["_is_mdr"].any() or not lot:
            continue
        data=pd.to_datetime(date_value,errors="coerce")
        if pd.isna(data): continue
        drug=float(group.loc[group["_is_mdr"],"_component_qty"].sum())
        output=float(group["_output_qty"].max())
        malto=float(group.loc[group["_is_malto"],"_component_qty"].sum())
        pure=max(output-malto,0)
        if drug<=0 or output<=0: continue
        drug_codes="; ".join(dict.fromkeys(group.loc[group["_is_mdr"],"_component"].astype(str)))
        drug_lots="; ".join(dict.fromkeys(
            x for x in group.loc[group["_is_mdr"],e_component_lot].astype(str) if x.strip()
        ))
        rows.append({
            "id":f"HIST-COMBER-{str(order).strip()}-{str(row_no).strip()}-{lot}",
            "id_turno":f"STORICO-{data:%Y%m%d}-COMBER","data_turno":data.date().isoformat(),
            "turno":"","macchina":"Comber","lotto":lot,"codice_droga":drug_codes,
            "codice_semilavorato":article,"descrizione":str(description).strip(),
            "kg_droga":drug,"kg_liquido":0,"residuo_secco_pct":0,"kg_puro":pure,
            "kg_semilavorato":0,"pct_puro_semilavorato":0,
            "kg_puro_equivalente":max(pure,drug*0.15),"lotti_comber":drug_lots,"piano_id":"",
            "note":f"STORICO_GESTIONALE_COMBER | Commessa {str(order).strip()} riga {str(row_no).strip()} | Prodotto secco {output:.3f} kg | Maltodestrina {malto:.3f} kg",
        })
        comber_count+=1
    history=pd.DataFrame(rows,columns=COL_LOTTI)
    diagnostics={
        "spray_rows":len(atom),"spray_kg":float(atom["_qty"].sum()),
        "spray_real_cut":taglio_reale,"comber_rows":comber_count,
        "comber_pure":float(pd.to_numeric(history.loc[history["macchina"]=="Comber","kg_puro"],errors="coerce").fillna(0).sum()),
        "history_cutoff":history_cutoff.date().isoformat(),
    }
    return history,diagnostics


def save_historical_sources(history):
    existing=read_csv(LOTTI_FILE,COL_LOTTI)
    historical_mask=(
        existing["note"].astype(str).str.startswith("STORICO_GESTIONALE_")|
        existing["note"].astype(str).str.strip().eq("Importato dallo storico EVRA")|
        existing["id"].astype(str).str.startswith(("H-S-","H-C-"))
    )
    manual=existing[~historical_mask].copy()
    manual_keys=set(
        manual["macchina"].astype(str)+"|"+manual["lotto"].astype(str).str.strip()
    )
    history_keys=history["macchina"].astype(str)+"|"+history["lotto"].astype(str).str.strip()
    imported=history[~history_keys.isin(manual_keys)].copy()
    skipped=len(history)-len(imported)
    save_csv(pd.concat([manual,imported],ignore_index=True),LOTTI_FILE,COL_LOTTI)
...
def parse_comber_plan(upload):
    ...
    for pos,i in enumerate(starts):
        ...
        product_key=normalize_text(product).replace(" ","_") or f"RIGA_{pos+1}"
        plan_id=f"{year}-W{int(float(week)):02d}-{product_key}-{pos+1}"
        plans.append({
...
def _effectiveness_targets():
    return {machine:annual_day_target(machine,"equivalente") for machine in PROCESS_MACHINES}


def _loss_pareto(events, causes, mode):
    if events.empty: return pd.DataFrame(columns=["Causale","Ore"])
    active=events[events["tipo_evento"]!="Produzione"].copy()
    active["Ore"]=pd.to_numeric(active["durata_h"],errors="coerce").fillna(0)
    lookup_id={str(r.cause_id):r for r in causes.itertuples() if str(r.cause_id).strip()}
    lookup_name={normalize_text(r.causale):r for r in causes.itertuples()}
    keep=[]
    for row in active.itertuples():
        cause=lookup_id.get(str(getattr(row,"cause_id","")).strip()) or lookup_name.get(normalize_text(row.tipo_evento))
        excluded=str(getattr(cause,"esclusa_pianificato","NO")).upper()=="SI" if cause else False
        keep.append(mode=="OOE" or not excluded)
    active=active[pd.Series(keep,index=active.index)]
    if active.empty: return pd.DataFrame(columns=["Causale","Ore"])
    return active.groupby("tipo_evento",as_index=False)["Ore"].sum().rename(columns={"tipo_evento":"Causale"}).sort_values("Ore",ascending=True)


def render_open_productions(events):
    st.subheader("Produzioni attualmente in corso")
    st.caption("Stato ricostruito da tutti i turni salvati, indipendentemente dal periodo selezionato per OEE/OOE.")
    open_rows=open_productions(events)
    counts={machine:int((open_rows["Macchina"]==machine).sum()) if not open_rows.empty else 0 for machine in PROCESS_MACHINES}
    st.caption(
        f'{len(open_rows)} lavorazioni aperte · Comber {counts["Comber"]} · '
        f'EV200 {counts["EV200"]} · Spray Dryer {counts["Spray Dryer"]}'
    )
    columns=st.columns(3)
    for column,machine in zip(columns,PROCESS_MACHINES):
        with column:
            st.markdown(f"#### {machine}")
            machine_rows=open_rows[open_rows["Macchina"]==machine].drop(columns=["Macchina"],errors="ignore")
            if machine_rows.empty:
                st.success("Nessuna produzione aperta")
            else:
                st.dataframe(machine_rows,use_container_width=True,hide_index=True)


def render_effectiveness_dashboard(events,productions,causes,start,end,mode):
    results=calculate_effectiveness(events,productions,causes,_effectiveness_targets(),QUALITA,start,end)
    selected_metric="OEE" if mode=="OEE" else "OOE"
    if mode=="Confronto":
        table=pd.DataFrame([{
            "Macchina":r["Macchina"],"OEE":r["OEE"]*100,"OOE":r["OOE"]*100,
            "Differenza p.p.":(r["OEE"]-r["OOE"])*100,"Ore escluse OEE":r["Ore escluse OEE"],
        } for r in results])
        st.dataframe(table,use_container_width=True,hide_index=True,column_config={
            "OEE":st.column_config.ProgressColumn("OEE",min_value=0,max_value=100,format="%.1f%%"),
            "OOE":st.column_config.ProgressColumn("OOE",min_value=0,max_value=100,format="%.1f%%"),
            "Differenza p.p.":st.column_config.NumberColumn("Differenza p.p.",format="%.1f"),
            "Ore escluse OEE":st.column_config.NumberColumn("Attività fuori OEE (h)",format="%.2f"),
        })
    else:
        cols=st.columns(3)
        for col,result in zip(cols,results):
            with col:
                st.plotly_chart(gauge(result[selected_metric],f'{selected_metric} {result["Macchina"]}'),use_container_width=True)
                c1,c2,c3=st.columns(3)
                availability=result[f"Availability {selected_metric}"]
                c1.metric("Availability",f"{availability:.1%}")
                c2.metric("Performance",f'{result["Performance"]:.1%}')
                c3.metric("Quality",f'{result["Qualità"]:.1%}')
                with st.expander("Dettaglio del calcolo"):
                    st.metric("Ore complessive",f'{result["Ore totali"]:.2f}')
                    st.metric("Attività escluse dall'OEE",f'{result["Ore escluse OEE"]:.2f}')
                    st.metric("Fermi inclusi nell'OEE",f'{result["Perdite OEE"]:.2f}')
                    st.metric("Tempo operativo",f'{result["Tempo operativo"]:.2f}')
                    st.metric("Output equivalente",f'{result["Output equivalente"]:.1f} kg')
                    st.metric("Output teorico nel tempo operativo",f'{result["Output teorico"]:.1f} kg')
                    if result["Ore causali non riconosciute"]>0:
                        st.warning(f'{result["Ore causali non riconosciute"]:.2f} ore hanno causali non riconosciute e sono considerate perdite OEE.')
                if result["Performance grezza"]>1.05:
                    st.warning(f'Output superiore allo standard teorico ({result["Performance grezza"]:.1%}); la Performance è limitata al 100%.')
    render_open_productions(events)
    period_events=events.copy()
    period_events["_date"]=pd.to_datetime(period_events["data_turno"],errors="coerce").dt.date
    period_events=period_events[(period_events["_date"]>=start)&(period_events["_date"]<=end)&period_events["macchina"].isin(PROCESS_MACHINES)]
    pareto_mode="OOE" if mode in ["OOE","Confronto"] else "OEE"
    losses=_loss_pareto(period_events,causes,pareto_mode)
...
cause_config(persist_legacy=True)
...
if page=="Pianificazione Comber":
    ...
    if uploaded_plan:
        try:
            ...
            declared=int(float(preview.iloc[0]["settimana"]))
            actual_weeks={pd.Timestamp(value).isocalendar().week for value in preview["data_inizio"]}
            inconsistent=any(int(value)!=declared for value in actual_weeks)
            if inconsistent:
                st.warning(f"Controllo date: il file dichiara la settimana {declared}, ma le date appartengono a {', '.join('W'+str(x) for x in sorted(actual_weeks))}. Verifica il file prima di importare.")
            ...
            confirm_plan=st.checkbox("Confermo l’importazione del piano Comber e la sostituzione dell'eventuale piano della stessa settimana")
            if st.button("Importa pianificazione",type="primary",disabled=not confirm_plan):
                existing=read_csv(PLANNING_FILE,COL_PLANNING)
                preview_years=set(pd.to_datetime(preview["data_inizio"],errors="coerce").dt.year.dropna().astype(int))
                existing_years=pd.to_datetime(existing["data_inizio"],errors="coerce").dt.year
                same_period=existing["settimana"].astype(str).isin(preview["settimana"].astype(str)) & existing_years.isin(preview_years)
                old_ids=existing.loc[same_period,"piano_id"].astype(str).tolist()
                if old_ids: soft_delete_ids(PLANNING_FILE,old_ids)
                existing=existing[~same_period]
    ...
    if plans.empty:
        ...
        plan_years=pd.to_datetime(plans["data_inizio"],errors="coerce").dt.year.fillna(date.today().year).astype(int)
        plan_weeks=pd.to_numeric(plans["settimana"],errors="coerce").fillna(0).astype(int)
        plans=plans.copy(); plans["_week_key"]=[f"{year}-W{week:02d}" for year,week in zip(plan_years,plan_weeks)]
        weeks=sorted([x for x in plans["_week_key"].unique() if str(x).strip()],reverse=True)
        ...
        current=plans[plans["_week_key"]==selected_week].drop(columns=["_week_key"])
        plans=plans.drop(columns=["_week_key"])
        events=read_csv(EVENTI_FILE,COL_EVENTI)
        progress,backlog,unplanned=comber_planning_view(plans,events,current)
        total_plan=float(pd.to_numeric(progress["kg_pianificati"],errors="coerce").fillna(0).sum()) if not progress.empty else 0
        total_done=float(progress["kg_effettivi"].sum()) if not progress.empty else 0
        total_residual=float(progress["kg_residui"].sum()) if not progress.empty else 0
        backlog_open=float(backlog["kg_residui_inizio_settimana"].sum()) if not backlog.empty else 0
        k1,k2,k3,k4,k5=st.columns(5)
        k1.metric("Nuovo piano",f"{fmt_it(total_plan)} kg")
        k2.metric("Attribuiti al piano",f"{fmt_it(total_done)} kg",delta=f"{pct(total_done,total_plan):.1f}%" if total_plan else None)
        k3.metric("Residuo piano",f"{fmt_it(total_residual)} kg")
        k4.metric("Backlog iniziale",f"{fmt_it(backlog_open)} kg")
        k5.metric("Fuori piano/extra",f"{fmt_it(unplanned['kg_droga'].sum() if not unplanned.empty else 0)} kg")

        if not backlog.empty:
            st.subheader("Backlog dalle settimane precedenti")
            backlog["stato_visuale"]=backlog["semaforo"]+" "+backlog["stato"]
            st.dataframe(backlog[["stato_visuale","settimana","prodotto","kg_residui_inizio_settimana","kg_recuperati_settimana","kg_residui"]],use_container_width=True,hide_index=True,column_config={
                "stato_visuale":"Stato","settimana":"Settimana origine","prodotto":"Droga",
                "kg_residui_inizio_settimana":st.column_config.NumberColumn("Backlog iniziale kg",format="%.2f"),
                "kg_recuperati_settimana":st.column_config.NumberColumn("Recuperati kg",format="%.2f"),
                "kg_residui":st.column_config.NumberColumn("Residuo attuale kg",format="%.2f"),
            })
        st.subheader("Piano della settimana")
        progress["stato_visuale"]=progress["semaforo"]+" "+progress["stato"]
        view=progress[["stato_visuale","data_inizio","prodotto","lotto_droga","kg_pianificati","kg_effettivi","kg_residui","estrazioni_pianificate","estrazioni_completate","avanzamento_grafico_pct","avanzamento_atteso_pct"]]
        ...
        if not unplanned.empty:
            st.warning("Produzioni non attribuite o eccedenti il piano selezionato")
            st.dataframe(unplanned,use_container_width=True,hide_index=True,column_config={"kg_droga":st.column_config.NumberColumn("Kg non attribuiti",format="%.2f")})
        else:
            st.success("Tutte le produzioni completate risultano attribuite alla pianificazione.")
    ...
    st.caption("Comber, EV200 e Spray Dryer · qualità standard temporanea 95% · classificazione causali retroattiva.")
    today=date.today(); default_start=today.replace(day=1)
    f1,f2,f3=st.columns([1,1,1.4])
    with f1: start=st.date_input("Dal",value=default_start,format="DD/MM/YYYY",key="eff_start")
    with f2: end=st.date_input("Al",value=today,format="DD/MM/YYYY",key="eff_end")
    with f3: mode=st.radio("Indicatore",["OEE","OOE","Confronto"],horizontal=True,key="eff_mode")
    if end<start: st.error("La data finale deve essere successiva alla data iniziale."); st.stop()
    all_events=read_csv(EVENTI_FILE,COL_EVENTI); all_prod=prep_production(); causes=cause_config()
    if mode=="OEE":
        st.info("OEE: misura l'efficienza nel solo tempo pianificato per produrre. Attesa prodotto, lavaggi e altre attività escluse non lo penalizzano.")
    elif mode=="OOE":
        st.info("OOE: misura l'utilizzo nell'intero tempo operativo registrato. Include anche attese e attività programmate escluse dall'OEE.")
    else:
        st.info("Confronto: la differenza OEE–OOE evidenzia l'impatto delle attività non pianificate come produzione.")
    render_effectiveness_dashboard(all_events,all_prod,causes,start,end,mode)
    with st.expander("Come vengono calcolati OEE e OOE"):
        explanation=Path("assets")/"oee_ooe_explainer.jpeg"
        if explanation.exists(): st.image(str(explanation),use_column_width=True)
        st.markdown("""
        **OEE = Availability OEE × Performance × Quality**  
        L'Availability OEE usa come denominatore soltanto produzione e fermi avvenuti mentre era pianificato produrre.

        **OOE = Availability OOE × Performance × Quality**  
        L'Availability OOE usa tutte le ore operative registrate, includendo attesa prodotto, lavaggi, cambi lotto e manutenzioni programmate.

        La Performance confronta l'output equivalente con quello teorico nel solo tempo effettivo di produzione; in questo modo i fermi non vengono penalizzati due volte.
        """)
    ...
    with tab2:
        with st.form("new_cause"):
            a,b=st.columns(2)
            with a:
                cause=st.text_input("Nuova causale")
                cat=st.selectbox("Categoria",["Operativa","Organizzativa","Organizzativa/Qualità","Tecnica","Fermo programmato","Utilities"])
            with b:
                excluded=st.checkbox("Fuori dal tempo pianificato OEE",help="Se attivo, la causale non penalizza l'OEE ma resta visibile nell'OOE.")
                technical=st.checkbox("Perdita tecnica")
                mandatory_note=st.checkbox("Nota obbligatoria")
            addc=st.form_submit_button("Aggiungi causale",type="primary")
        if addc and cause.strip():
            c=cause_config(); c=c[c["causale"]!=cause.strip()]
            c=pd.concat([c,pd.DataFrame([{
                "cause_id":cause_id_for_name(cause.strip()),"causale":cause.strip(),"categoria":cat,
                "esclusa_pianificato":"SI" if excluded else "NO","penalizza_ooe":"SI",
                "perdita_tecnica":"SI" if technical else "NO","nota_obbligatoria":"SI" if mandatory_note else "NO","attiva":"SI",
            }])],ignore_index=True)
            save_csv(c,CAUSALI_FILE,COL_CAUSALI); st.success("Causale salvata."); st.rerun()
        original_causes=cause_config()
        st.caption("Fuori dal tempo pianificato OEE = non penalizza l'OEE, ma penalizza l'OOE. Le modifiche si applicano retroattivamente agli eventi collegati.")
        editedc=st.data_editor(
            original_causes,use_container_width=True,num_rows="fixed",disabled=["cause_id"],key="causes",
            column_config={
                "cause_id":"ID stabile","causale":"Causale","categoria":"Categoria",
                "esclusa_pianificato":st.column_config.SelectboxColumn("Fuori OEE",options=["SI","NO"],required=True),
                "penalizza_ooe":st.column_config.SelectboxColumn("Penalizza OOE",options=["SI","NO"],required=True),
                "perdita_tecnica":st.column_config.SelectboxColumn("Perdita tecnica",options=["SI","NO"],required=True),
                "nota_obbligatoria":st.column_config.SelectboxColumn("Nota obbligatoria",options=["SI","NO"],required=True),
                "attiva":st.column_config.SelectboxColumn("Attiva",options=["SI","NO"],required=True),
            },
        )
        changed=not editedc.fillna("").astype(str).equals(original_causes.fillna("").astype(str))
        if changed:
            events_preview=read_csv(EVENTI_FILE,COL_EVENTI); prod_preview=prep_production()
            before=calculate_effectiveness(events_preview,prod_preview,original_causes,_effectiveness_targets(),QUALITA)
            after=calculate_effectiveness(events_preview,prod_preview,editedc,_effectiveness_targets(),QUALITA)
            impact=pd.DataFrame([{
                "Macchina":old["Macchina"],"OEE prima":old["OEE"]*100,"OEE dopo":new["OEE"]*100,
                "Variazione p.p.":(new["OEE"]-old["OEE"])*100,"OOE":new["OOE"]*100,
            } for old,new in zip(before,after)])
            st.warning("La modifica ricalcolerà retroattivamente gli indicatori. Anteprima sull'intero storico:")
            st.dataframe(impact,use_container_width=True,hide_index=True,column_config={
                "OEE prima":st.column_config.NumberColumn(format="%.1f%%"),"OEE dopo":st.column_config.NumberColumn(format="%.1f%%"),
                "Variazione p.p.":st.column_config.NumberColumn(format="%.1f"),"OOE":st.column_config.NumberColumn(format="%.1f%%"),
            })
        confirm_causes=st.checkbox("Confermo la riclassificazione retroattiva delle causali",disabled=not changed)
        if st.button("Salva modifiche causali",type="primary",disabled=not(changed and confirm_causes)):
            linked,_=save_cause_changes(original_causes,editedc)
            st.success(f"Causali aggiornate. Collegati retroattivamente {linked} eventi storici privi di ID causale."); st.rerun()
    ...
    st.divider()
    st.subheader("Importa storico gestionale")
    st.caption(
        "Carico Produzione ricostruisce l'essiccazione dalle righe ATOM; "
        "Esplosione commessa ricostruisce puro, droga MDR, maltodestrina e tracciabilità."
    )
    h1,h2=st.columns(2)
    with h1:
        carico_file=st.file_uploader(
            "File Carico Produzione",type=["xlsx"],key="historical_carico",
            help="Sono utilizzate esclusivamente le righe con Lavorazione = ATOM.",
        )
    with h2:
        explosion_file=st.file_uploader(
            "File Esplosione commessa",type=["xlsx"],key="historical_explosion",
        )
    if carico_file and explosion_file:
        try:
            historical_preview,historical_info=parse_historical_sources(carico_file,explosion_file)
            p1,p2,p3,p4=st.columns(4)
            p1.metric("Lotti Spray Dryer",historical_info["spray_rows"])
            p2.metric("Kg essiccati",fmt_it(historical_info["spray_kg"]))
            p3.metric("Lotti Comber",historical_info["comber_rows"])
            p4.metric("Puro Estrazione",fmt_it(historical_info["comber_pure"]))
            st.info(
                f'Taglio reale ricostruito per {historical_info["spray_real_cut"]} lotti Spray Dryer. '
                "Negli altri casi viene applicata la base standard del 40% e il dato resta identificato nelle note. "
                f'Storico consolidato fino al {pd.to_datetime(historical_info["history_cutoff"]):%d/%m/%Y}.'
            )
            preview_columns=[
                "data_turno","macchina","lotto","codice_droga","codice_semilavorato",
                "descrizione","kg_droga","kg_puro","kg_semilavorato","pct_puro_semilavorato",
                "kg_puro_equivalente",
            ]
            st.dataframe(
                historical_preview[preview_columns].tail(100).iloc[::-1],
                use_container_width=True,hide_index=True,
            )
            confirm_history=st.checkbox(
                "Confermo l'aggiornamento dello storico gestionale",
                help="I dati gestionali già importati vengono sostituiti. Eventi e produzioni inseriti manualmente restano invariati.",
            )
            if st.button("Importa storico gestionale",type="primary",disabled=not confirm_history):
                imported,skipped=save_historical_sources(historical_preview)
                st.success(
                    f"Storico aggiornato: {imported} produzioni importate. "
                    f"{skipped} lotti non importati perché già presenti come dati manuali."
                )
                st.rerun()
        except Exception as exc:
            st.error(f"Impossibile ricostruire lo storico gestionale: {exc}")
