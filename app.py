...
from oee_analytics import (
    ...
    latest_machine_productions,
    normalize_text,
    open_productions,
)

# RELEASE OEE LAURIA 4.0.6 — ultima lavorazione live per macchina
...
def render_open_productions(events):
    st.subheader("Lavorazione attuale / ultima registrata")
    st.caption("Per ogni macchina è mostrato l'evento produttivo più recente salvato.")
    open_rows=latest_machine_productions(events)
    open_rows=open_rows[open_rows["Macchina"].isin(EFFECTIVENESS_MACHINES)]
    ...
    for column,machine in zip(columns,EFFECTIVENESS_MACHINES):
        with column:
            ...
            if machine_rows.empty:
                st.info("Nessuna lavorazione registrata")
            else:
