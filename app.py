...
# RELEASE OEE LAURIA 4.0.4 — lotti aperti visibili fino alla chiusura
...
def render_open_productions(events):
    ...
    st.caption("Sono mostrati tutti i lotti aperti nei turni salvati; restano visibili fino alla registrazione della chiusura.")
    open_rows=open_productions(events)
    open_rows=open_rows[
        open_rows["Macchina"].isin(EFFECTIVENESS_MACHINES)
        & (open_rows["Stato"]=="🟢 IN LAVORAZIONE")
    ]
    counts={machine:int((open_rows["Macchina"]==machine).sum()) if not open_rows.empty else 0 for machine in EFFECTIVENESS_MACHINES}
    st.caption(
        f'{len(open_rows)} lavorazioni aperte · Comber {counts["Comber"]} · '
        f'Spray Dryer {counts["Spray Dryer"]}'
    )
    columns=st.columns(2)
    for column,machine in zip(columns,EFFECTIVENESS_MACHINES):
        with column:
...
def render_effectiveness_dashboard(events,productions,causes,start,end,mode):
    ...
    results=[result for result in results if result["Macchina"] in EFFECTIVENESS_MACHINES]
    period_productions=productions.copy()
    if not period_productions.empty:
        period_productions["_date"]=pd.to_datetime(period_productions["data_turno"],errors="coerce").dt.date
        period_productions=period_productions[
            (period_productions["_date"]>=start)&(period_productions["_date"]<=end)
        ]
    ...
    if mode=="Confronto":
        ...
        for col,result in zip(cols,results):
            with col:
                ...
                if result["Macchina"]=="Comber":
                    mass_yield=process_indicator(period_productions,"Comber")
                    c3.metric("Mass Yield media",f"{mass_yield:.1f}%")
                else:
                ...
                with st.expander("Dettaglio del calcolo"):
                    ...
                    st.metric("Quality usata nel calcolo",f'{result["Qualità"]:.1%}')
    ...
    period_events=period_events[(period_events["_date"]>=start)&(period_events["_date"]<=end)&period_events["macchina"].isin(EFFECTIVENESS_MACHINES)]
...
if page=="Pianificazione Comber":
    ...
    if plans.empty:
        ...
        k1.metric("Piano della settimana",f"{fmt_it(total_plan)} kg")
        k2.metric("Attribuiti al piano",f"{fmt_it(total_done)} kg",delta=f"{pct(total_done,total_plan):.1f}%" if total_plan else None)
        k3.metric("Residuo della settimana",f"{fmt_it(total_residual)} kg")
