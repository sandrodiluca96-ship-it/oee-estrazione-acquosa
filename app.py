...
# RELEASE OEE LAURIA 3.11.1 — scarico Comber autonomo e lotti EV200 corretti
...
def save_history_event_edits(original,edited):
    ...
    for shift_id in affected_shifts:
        ...
        productive=shift_events["tipo_evento"].isin(["Produzione","Scarico estrattore"])
        production_hours=pd.to_numeric(
            shift_events.loc[productive,"durata_h"],errors="coerce"
        ).fillna(0).sum()
...
def delete_history_events(event_ids):
    ...
    for shift_id in removed["id_turno"].astype(str).unique():
        ...
        if remaining.empty:
            ...
            continue
        productive=remaining["tipo_evento"].isin(["Produzione","Scarico estrattore"])
        production_hours=pd.to_numeric(
            remaining.loc[productive,"durata_h"],errors="coerce"
        ).fillna(0).sum()
        idx=turns["id_turno"].astype(str)==shift_id
        if idx.any():
            turns.loc[idx,"ore_produzione"]=float(production_hours)
