...
# RELEASE OEE LAURIA 3.4.0 — EV200 semplificato e planning Comber
...
def equivalent_spray(kg_semi, kg_pure):
    # Standard 60/40: con almeno il 40% di puro si conserva il prodotto reale;
    # con un taglio maggiore si riporta il puro all'equivalente fisico al 40%.
    if kg_semi <= 0: return 0.0
    return min(kg_semi, kg_pure / 0.40)
...
def prep_production():
    ...
    df.loc[spray,"kg_puro_equivalente"] = pd.concat(
        [df.loc[spray,"kg_semilavorato"],df.loc[spray,"kg_puro"].div(0.40)],axis=1
    ).min(axis=1)
    return df
...
area=st.sidebar.radio("Area",[
    "Comber","Pianificazione Comber","EV200","Spray Dryer",
...
if page=="Pianificazione Comber":
    ...
    with st.form("production"):
        ...
        if machine=="Comber":
            ...
            st.info(f"Puro ottenuto: {kg_pure:.1f} kg · Semilavorato equivalente 40%: {equivalent_spray(kg_semi,kg_pure):.1f} kg")
