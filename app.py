...
# RELEASE OEE LAURIA 3.9.1 — semaforo giornaliero Comber
...
def _clock(value, fallback):
    text=str(value).strip().split("-")[0].strip().replace("24:00","00:00")
...
if page=="Pianificazione Comber":
    ...
    if plans.empty:
        ...
        first=pd.to_datetime(current["data_inizio"],errors="coerce").min().date()
        last=pd.to_datetime(current["data_fine"],errors="coerce").max().date()
        default_day=min(max(date.today(),first),last)
        selected_day=st.date_input("Giorno da controllare",value=default_day,min_value=first,max_value=last)
        daily=comber_daily_status(current,selected_day)

        message=f'{daily["semaforo"]} {daily["stato"]} · avanzamento {daily["avanzamento_pct"]:.0f}% · atteso {daily["atteso_pct"]:.0f}%'
        if daily["stato"]=="IN RITARDO": st.error(message)
        elif daily["stato"]=="SOTTO IL RITMO ATTESO": st.warning(message)
        elif daily["stato"]=="NESSUN PIANO": st.info(message)
        else: st.success(message)

        k1,k2,k3,k4,k5=st.columns(5)
        k1.metric("Kg pianificati",f'{daily["kg_pianificati"]:,.0f}'.replace(",","."))
        k2.metric("Kg completati",f'{daily["kg_effettivi"]:,.0f}'.replace(",","."))
        k3.metric("Estrazioni pianificate",f'{daily["estrazioni_pianificate"]:.1f}'.rstrip("0").rstrip("."))
        k4.metric("Estrazioni completate",daily["estrazioni_completate"])
        k5.metric("Estrazioni in corso",daily["estrazioni_in_corso"])

        active=daily["piani"].copy()
        if active.empty:
            st.info("Per il giorno selezionato non risultano lavorazioni pianificate.")
        else:
            active["semaforo"]=daily["semaforo"]+" "+daily["stato"]
            active["avanzamento_atteso_pct"]=daily["atteso_pct"]
            active["kg_completati_giorno"]=daily["kg_effettivi"]
            active["estrazioni_completate_giorno"]=daily["estrazioni_completate"]
            view=active[["semaforo","prodotto","lotto_droga","ora_inizio","ora_fine","kg_giorno","kg_completati_giorno","estrazioni_giorno","estrazioni_completate_giorno","avanzamento_atteso_pct"]]
            st.dataframe(view,use_container_width=True,hide_index=True,column_config={
                "semaforo":"Stato del giorno","prodotto":"Prodotto","lotto_droga":"Lotto droga",
                "ora_inizio":"Inizio previsto","ora_fine":"Fine prevista",
                "kg_giorno":st.column_config.NumberColumn("Kg pianificati",format="%.1f"),
                "kg_completati_giorno":st.column_config.NumberColumn("Kg completati",format="%.1f"),
                "estrazioni_giorno":st.column_config.NumberColumn("Estrazioni pianificate",format="%.1f"),
                "estrazioni_completate_giorno":"Estrazioni completate",
                "avanzamento_atteso_pct":st.column_config.ProgressColumn("Avanzamento atteso",min_value=0,max_value=100,format="%.0f%%"),
            })
        if daily["lotti_fuori_piano"]:
            st.error("🔴 Lotti inseriti fuori pianificazione: "+", ".join(daily["lotti_fuori_piano"]))
            extra=daily["eventi_fuori_piano"][["lotto","numero_estrazione","descrizione","ora_inizio","ora_fine","stato_estrazione"]].copy()
            st.dataframe(extra,use_container_width=True,hide_index=True)
        else:
            st.success("Nessun lotto fuori pianificazione nel giorno selezionato.")
