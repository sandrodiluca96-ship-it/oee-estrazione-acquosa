...
FASI_COMBER = ["Apertura lotto", "Prosecuzione lotto", "Chiusura lotto"]
EVENTI_PRODUTTIVI = {"Produzione", "Scarico estrattore"}

COL_EVENTI = [
    "id_evento", "id_turno", "data_turno", "turno", "macchina", "tipo_evento",
    "tipo_produzione", "lotto", "codice", "descrizione", "ora_inizio", "ora_fine",
    "durata_h", "numero_estrazione", "stato_lotto", "stato_estrazione", "fase_lavorazione",
    "numero_operatori", "quantita_pianificata", "quantita_finale", "ore_uomo",
    "kg_droga", "kg_liquido", "rs_liquido_pct", "kg_liquido_alimentato",
...
def _comber_extraction_summary(rows):
    """Consolida le estrazioni Comber senza duplicare i dati tra più turni."""
    numbered={}
    legacy=[]
    for r in rows:
        extraction=str(r.get("numero_estrazione","")).strip()
        if not extraction:
            legacy.append(r)
            continue
        numbered.setdefault(extraction,[]).append(r)

    drug=liquid=pure=0.0
    for extraction_rows in numbered.values():
        completed=[r for r in extraction_rows if r.get("stato_estrazione")=="Completata"]
        if not completed:
            continue
        # Il target considera soltanto le estrazioni completate. La droga può
        # essere riportata anche nella prosecuzione del turno: il massimo per
        # numero estrazione evita il doppio conteggio.
        drug += max([_number(r.get("kg_droga")) for r in extraction_rows]+[0.0])
        final=max(completed,key=lambda r:_number(r.get("kg_liquido")))
        ext_liquid=_number(final.get("kg_liquido"))
        liquid += ext_liquid
        pure += ext_liquid*_number(final.get("rs_liquido_pct"))/100

    # Compatibilità con gli eventi creati prima dell'introduzione del numero
    # estrazione: si conserva la precedente logica di calcolo.
    if legacy:
        drug += _first(legacy,"kg_droga")
        liquid += sum(_number(r.get("kg_liquido")) for r in legacy)
        pure += sum(
            _number(r.get("kg_liquido"))*_number(r.get("rs_liquido_pct"))/100
            for r in legacy
        )
    return drug,liquid,pure
...
def _create_production(machine,lot,shift_id,data_turno,shift,buffer):
    ...
    if machine=="Comber":
        drug,liquid,pure=_comber_extraction_summary(rows)
        ...
        final=max([_number(r.get("quantita_finale")) for r in rows]+[0])
        planned=_first(rows,"quantita_pianificata")
        duration=sum(_number(r.get("durata_h")) for r in rows)
        person_hours=sum(_number(r.get("durata_h"))*_number(r.get("numero_operatori")) for r in rows)
        productivity=final/person_hours if person_hours else 0
        out.update({
            "codice_semilavorato":code,"kg_semilavorato":final,"piano_id":_first_text(rows,"piano_id"),
            "note":f"Pianificato: {planned:.1f} kg · Durata: {duration:.2f} h · Ore-uomo: {person_hours:.2f} · Produttività: {productivity:.1f} kg/ora-uomo",
        })
    return out
...
def _product_options(machine):
    ...
    kind="Mescola" if machine=="Mescole" else ("Semilavorato" if machine=="Spray Dryer" else "Droga")
    return products[products["tipo"]==kind].copy()


def _comber_lots_for_ev200():
    """Lotti creati nel flusso operativo Comber, esclusi i consuntivi storici."""
...
def _open_mescole_plans(work_day=None):
    plans=_read(MESCOLE_PLANNING_FILE,COL_MESCOLE_PLANNING)
    if plans.empty: return []
    work_day=work_day or datetime.today().date()
    iso=work_day.isocalendar(); current_week=f"{iso.year}-W{iso.week:02d}"
    due=pd.to_datetime(plans["data_prevista"],errors="coerce")
    derived=due.apply(lambda value:f"{value.isocalendar().year}-W{value.isocalendar().week:02d}" if pd.notna(value) else "")
    plans.loc[plans["settimana"].astype(str).str.strip()=="","settimana"]=derived
    events=_read(EVENTI_FILE,COL_EVENTI)
    produced=(events[(events["macchina"]=="Mescole")&(events["tipo_produzione"]=="Chiusura lotto")]
              .groupby("piano_id")["quantita_finale"].apply(lambda x: sum(_number(v) for v in x)).to_dict())
    open_plans=[p for p in plans.to_dict("records") if produced.get(p["piano_id"],0)<_number(p["quantita_pianificata"])]
    same_week=[p for p in open_plans if p.get("settimana")==current_week]
    return same_week or open_plans
...
def _product_selector(machine,state,reset,current_code="",key_suffix=""):
    ...
    field_label={"Comber":"Codice droga e descrizione *","EV200":"Codice prodotto e descrizione *","Spray Dryer":"Codice semilavorato e descrizione *","Mescole":"Codice prodotto da miscelare *"}[machine]
    options=["-- Seleziona --"]+labels+["＋ Aggiungi nuovo codice"]
    ...
    if selected=="＋ Aggiungi nuovo codice":
        a,b=st.columns(2)
        with a: code=st.text_input("Nuovo codice *",key=f"new_code_{state}_{reset}_{key_suffix}").strip()
        with b: description=st.text_input("Nuova descrizione *",key=f"new_desc_{state}_{reset}_{key_suffix}").strip()
        st.caption("Il nuovo prodotto sarà salvato nell’anagrafica del reparto insieme all’evento.")
...
def render_machine_workflow(machine):
    ...
    special_events=["Scarico estrattore"] if machine=="Comber" else []
    active_causes=[cause for cause in active_causes if cause not in special_events]
    event_types=["Produzione"]+special_events+active_causes
    ...
    if event_type=="Produzione":
        ...
        if machine=="Mescole":
            actions=["Nuovo lotto"]+(["Riprendi lotto in corso"] if open_lots else [])
            default_action="Riprendi lotto in corso" if default_stage in ["Prosecuzione lotto","Chiusura lotto"] and open_lots else "Nuovo lotto"
        ...
        if stage=="Apertura lotto":
            if machine=="EV200":
                lot=""
                old_from_comber=bool(current and str(current.get("lotti_comber","")).strip())
                origin=st.radio(
                    "Origine del liquido da concentrare",["Da Comber","Altro lotto"],
                    index=0 if old_from_comber or not current else 1,horizontal=True,
                    key=f"ev200_origin_{state}_{s['reset']}",
                )
                if origin=="Da Comber":
                    ...
                    if selected and same_product:
                        ...
                        lot=row["lotti_comber"]
                        st.success(f"Lotto e prodotto ereditati: {lot} · {code} — {description}")
                    else:
                        code=description=""
                        if selected: st.error("I lotti selezionati appartengono a prodotti differenti.")
                else:
                    lot=st.text_input("Lotto da concentrare *",value=current.get("lotto","") if current else "",key=f"lot_{state}_{s['reset']}")
                    old_code=current.get("codice","") if current else ""
                    code,description=_product_selector(machine,state,s["reset"],old_code,"external")
            elif machine=="Mescole":
                lot=st.text_input("Lotto *",value=current.get("lotto","") if current else "",key=f"lot_{state}_{s['reset']}")
                plans=_open_mescole_plans(day)
                old_code=current.get("codice","") if current else ""
                code,description=_product_selector(machine,state,s["reset"],old_code,"opening")
                matching=sorted(
                    [p for p in plans if str(p.get("codice","")).strip()==code],
                    key=lambda p:(str(p.get("data_prevista","") or "9999-12-31"),str(p.get("piano_id",""))),
                )
                if matching:
                    plan=matching[0]
                    row["piano_id"]=plan["piano_id"]
                    row["quantita_pianificata"]=_number(plan["quantita_pianificata"])
                    st.caption(f"Pianificazione collegata automaticamente · {row['quantita_pianificata']:.1f} kg previsti")
            else:
                lot=st.text_input("Lotto *",value=current.get("lotto","") if current else "",key=f"lot_{state}_{s['reset']}")
                old_code=current.get("codice","") if current else ""
                code,description=_product_selector(machine,state,s["reset"],old_code,"opening")
            ...
            row["quantita_pianificata"]=_first(src,"quantita_pianificata")
            if lot: st.success(f"Lotto ripreso: {lot} · {_first_text(src,'codice')} · {_first_text(src,'descrizione')}")
        if machine=="Comber":
            lot_rows=_lot_source(machine,row["lotto"],buffer) if row["lotto"] else []
            existing_numbers=[int(_number(r.get("numero_estrazione"))) for r in lot_rows if _number(r.get("numero_estrazione"))>0]
            open_numbers=[]
            for extraction in existing_numbers:
                ext_rows=[r for r in lot_rows if int(_number(r.get("numero_estrazione")))==extraction]
                if not any(r.get("stato_estrazione")=="Completata" for r in ext_rows): open_numbers.append(extraction)
            suggested=open_numbers[-1] if open_numbers else (max(existing_numbers,default=0)+1)
            number_default=int(_number(current.get("numero_estrazione"))) if current and _number(current.get("numero_estrazione"))>0 else max(suggested,1)
            row["numero_estrazione"]=st.number_input(
                "Numero estrazione *",min_value=1,step=1,value=number_default,
                key=f"extraction_{state}_{s['reset']}",
            )
            extraction_rows=[r for r in lot_rows if int(_number(r.get("numero_estrazione")))==int(row["numero_estrazione"])]
            inherited_drug=max([_number(r.get("kg_droga")) for r in extraction_rows]+[0.0])
            drug_default=float(_number(current.get("kg_droga"))) if current and _number(current.get("kg_droga"))>0 else float(inherited_drug)
            row["kg_droga"]=st.number_input(
                "Droga caricata nella singola estrazione (kg) *",min_value=0.0,value=drug_default,
                help="Se l'estrazione prosegue dal turno precedente, la quantità viene ereditata e non sarà conteggiata due volte.",
            ...
            x,y=st.columns(2)
            with x:
                row["numero_operatori"]=st.selectbox("Numero operatori *",[1,2],index=max(0,int(_number(current.get("numero_operatori")) if current else 1)-1),key=f"operators_{state}_{s['reset']}")
            with y:
                row["quantita_finale"]=st.number_input("Quantità finale ottenuta (kg)",min_value=0.0,value=_number(current.get("quantita_finale")) if current else 0.0,help="Lascia 0 se la mescola non è conclusa: il lotto resterà automaticamente in corso.",key=f"mixqty_{state}_{s['reset']}")
            row["ore_uomo"]=hours*_number(row["numero_operatori"])
            completed=_number(row["quantita_finale"])>0
            row["tipo_produzione"]="Chiusura lotto" if completed else stage
            row["stato_lotto"]="Completato" if completed else "In corso"
            row["fase_lavorazione"]="Mescola completata" if completed else "Lavorazione in corso"
            st.caption(f"Stato lotto: {row['stato_lotto']} · Ore-uomo del segmento: {row['ore_uomo']:.2f}")
            if completed:
                st.success("La quantità chiude automaticamente il lotto. Sarà contabilizzata una sola volta.")
            else:
                st.info("Il lotto sarà disponibile nel turno successivo tra le lavorazioni da riprendere.")
    elif machine=="Comber" and event_type=="Scarico estrattore":
        open_lots=list(dict.fromkeys(
            _saved_open_lots(machine)+
            [r.get("lotto","") for r in buffer if r.get("tipo_produzione")=="Apertura lotto"]
        ))
        closed={r.get("lotto") for r in buffer if r.get("tipo_produzione")=="Chiusura lotto"}
        open_lots=[lot for lot in open_lots if lot and lot not in closed]
        old_lot=current.get("lotto","") if current else ""
        lot=st.selectbox(
            "Lotto Comber interessato *",
            open_lots,
            index=open_lots.index(old_lot) if old_lot in open_lots else 0 if open_lots else None,
            placeholder="Nessun lotto Comber aperto",
            key=f"discharge_lot_{state}_{s['reset']}",
        )
        source=_lot_source(machine,lot,buffer) if lot else []
        row.update({
            "lotto":lot or "",
            "codice":_first_text(source,"codice"),
            "descrizione":_first_text(source,"descrizione"),
            "piano_id":_first_text(source,"piano_id"),
            "tipo_produzione":"Scarico estrattore",
            "stato_lotto":"In corso",
            "fase_lavorazione":"Scarico estrattore",
        })
        if lot:
            st.success(
                f"Scarico collegato al lotto {lot} · "
                f"{row['codice']} — {row['descrizione']}"
            )
        else:
            st.info("Apri prima un lotto Comber per poter registrare lo scarico.")
    ...
    if add:
        if not event_type or not start or not end or hours<=0: st.error("Seleziona evento e orari validi.")
        elif event_type=="Scarico estrattore" and not row["lotto"]: st.error("Seleziona il lotto Comber interessato dallo scarico.")
        ...
        elif machine=="Comber" and event_type=="Produzione" and (_number(row["numero_estrazione"])<=0 or _number(row["kg_droga"])<=0): st.error("Inserisci il numero dell'estrazione e la droga caricata nella singola estrazione.")
        elif machine=="Comber" and event_type=="Produzione" and ((_number(row["kg_liquido"])>0) != (_number(row["rs_liquido_pct"])>0)): st.error("Inserisci sia la quantità di liquido sia il residuo secco, oppure lascia entrambi a zero per registrare la lavorazione in corso.")
        elif machine=="Comber" and event_type=="Produzione" and stage=="Chiusura lotto" and row["stato_estrazione"]!="Completata": st.error("Puoi chiudere il lotto soltanto dopo aver completato l'estrazione.")
        ...
        elif machine=="Mescole" and event_type=="Produzione" and _number(row["numero_operatori"]) not in [1,2]: st.error("Indica se la mescola è stata eseguita da 1 o 2 operatori.")
        else:
            ...
            if not valid: st.error(msg)
                ...
                if event_type=="Produzione" and row.get("codice"):
                    products=_read(PRODOTTI_FILE,COL_PRODOTTI)
                    kind="Mescola" if machine=="Mescole" else ("Semilavorato" if machine=="Spray Dryer" else "Droga")
                    if products[(products["tipo"]==kind)&(products["codice"]==row["codice"])].empty:
                        products=pd.concat([products,pd.DataFrame([{"tipo":kind,"codice":row["codice"],"descrizione":row["descrizione"],"pct_puro_standard":0,"attivo":"SI"}])],ignore_index=True)
                        _save(products,PRODOTTI_FILE,COL_PRODOTTI)
    ...
    if buffer:
        view_cols=["tipo_evento","tipo_produzione","lotto","codice","descrizione"]
        if machine=="Comber": view_cols += ["numero_estrazione","stato_lotto","stato_estrazione","fase_lavorazione"]
        if machine=="Mescole": view_cols += ["numero_operatori","quantita_pianificata","quantita_finale","stato_lotto","ore_uomo"]
        view_cols += ["ora_inizio","ora_fine","durata_h"]
        view=pd.DataFrame(buffer)[view_cols]
        ...
        if st.button("Salva turno",type="primary",disabled=not valid,key=f"save_{state}"):
            ...
            lots_to_update=[]
            for r in buffer:
                is_closed=r.get("tipo_produzione")=="Chiusura lotto"
                is_completed_extraction=machine=="Comber" and r.get("stato_estrazione")=="Completata"
                if r.get("tipo_evento")=="Produzione" and (is_closed or is_completed_extraction):
                    lots_to_update.append(r["lotto"])
            for lot in dict.fromkeys(lots_to_update):
                # Gli eventi del buffer sono già stati salvati: rileggere la
                # fonte evita di duplicare durata e quantità.
                prod=_create_production(machine,lot,shift_id,day,shift,[])
            ...
            prod_h=sum(_number(r["durata_h"]) for r in buffer if r.get("tipo_evento") in EVENTI_PRODUTTIVI); stop_h=8-prod_h
    ...
    if recent.empty:
        ...
        st.info("Per correggere un evento vai in Storico → Eventi di turno.")
