...
COL_EVENTI = [
    "id_evento", "id_turno", "data_turno", "turno", "macchina", "tipo_evento", "cause_id",
...
def _saved_open_lots(machine):
    ...
    return ordered_open_lots(ev,machine)


def _open_lots_with_buffer(machine, buffer):
    """Unisce i lotti salvati al turno corrente, mantenendo il più recente per primo."""
    active={normalize_text(lot):lot for lot in reversed(_saved_open_lots(machine)) if normalize_text(lot)}
    for row in buffer:
        if row.get("tipo_evento") != "Produzione":
            continue
        lot=str(row.get("lotto","")).strip()
        key=normalize_text(lot)
        if not key:
            continue
        if normalize_text(row.get("tipo_produzione",""))=="CHIUSURA LOTTO" or normalize_text(row.get("stato_lotto",""))=="COMPLETATO":
            active.pop(key,None)
        else:
            active.pop(key,None)
            active[key]=lot
    return list(reversed(active.values()))
...
def render_machine_workflow(machine):
    ...
    if event_type and event_type!="Produzione":
        selected_cause=causes[causes["causale"]==event_type]
        row["cause_id"]=str(selected_cause.iloc[0]["cause_id"]).strip() if not selected_cause.empty else cause_id_for_name(event_type)
    if event_type=="Produzione":
        open_lots=_open_lots_with_buffer(machine,buffer)
        ...
        if stage=="Apertura lotto":
            ...
            old_key=normalize_text(old_lot)
            selected_index=next((i for i,value in enumerate(options) if normalize_text(value)==old_key),0) if options else None
            lot=st.selectbox("Lotto aperto *",options,index=selected_index,placeholder="Nessun lotto aperto",key=f"openlot_{state}_{s['reset']}")
    ...
    if add:
        ...
        elif event_type!="Produzione" and not selected_cause.empty and str(selected_cause.iloc[0]["nota_obbligatoria"]).upper()=="SI" and not str(row["note"]).strip(): st.error("Per questa causale è obbligatorio inserire una nota.")
        else:
