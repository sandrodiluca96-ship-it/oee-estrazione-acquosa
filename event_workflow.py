...
COL_EVENTI = [
    ...
    "kg_maltodestrina", "kg_polvere_finale", "note",
]
COL_PRODUZIONI = [
...
def _create_production(machine,lot,shift_id,data_turno,shift,buffer):
    ...
    if machine=="Comber":
        ...
        out.update({"codice_droga":code,"kg_liquido":feed,"residuo_secco_pct":rs_final,"kg_puro":pure,"kg_puro_equivalente":pure})
    else:
        molle=_first(rows,"kg_molle"); rs=_first(rows,"rs_molle_pct"); malto=_first(rows,"kg_maltodestrina")
        final=max([_number(r.get("kg_polvere_finale")) for r in rows]+[0]); pure=molle*rs/100
        out.update({"codice_semilavorato":code,"kg_liquido":molle,"residuo_secco_pct":rs,"kg_puro":pure,"kg_semilavorato":final,"pct_puro_semilavorato":pure/final*100 if final else 0,"kg_puro_equivalente":pure,"note":f"Maltodestrina: {malto:.1f} kg"})
    return out
...
def _product_options(machine):
    ...
    kind="Droga" if machine=="Comber" else "Semilavorato"
...
def render_machine_workflow(machine):
    ...
    if event_type=="Produzione":
        ...
        if stage=="Apertura lotto":
            ...
            if machine=="EV200":
                comber=_read(PRODUZIONI_FILE,COL_PRODUZIONI); comber=comber[comber["macchina"]=="Comber"]
                labels=[f"{r.lotto} — {r.descrizione}" for r in comber.itertuples()]
                linked=st.selectbox("Lotto Comber di origine",["-- Inserimento manuale --"]+labels,key=f"link_{state}_{s['reset']}")
                if linked!="-- Inserimento manuale --":
                    lot=linked.split(" — ")[0]; src=comber[comber["lotto"]==lot].iloc[-1]; code=src["codice_droga"]; description=src["descrizione"]
                else: code=""; description=st.text_input("Descrizione estratto *",value=current.get("descrizione","") if current else "",key=f"desc_{state}_{s['reset']}")
            else:
                products=_product_options(machine); products["label"]=products["codice"]+" — "+products["descrizione"]
                labels=products["label"].tolist(); old_code=current.get("codice","") if current else ""
                old_label=next((x for x in labels if x.startswith(old_code+" — ")),None)
                selected=st.selectbox("Codice e descrizione *",["-- Seleziona --"]+labels,index=(["-- Seleziona --"]+labels).index(old_label) if old_label else 0,key=f"prod_{state}_{s['reset']}")
                if selected!="-- Seleziona --": code=selected.split(" — ")[0]; description=products[products["codice"]==code].iloc[0]["descrizione"]
            ...
            row.update({"lotto":lot or "","codice":_first_text(src,"codice"),"descrizione":_first_text(src,"descrizione")})
        ...
        if machine=="Comber":
            ...
            x,y=st.columns(2)
            with x: row["kg_liquido_alimentato"]=st.number_input("Liquido alimentato nell’evento (kg)",min_value=0.0,value=_number(current.get("kg_liquido_alimentato")) if current else 0.0,key=f"feed_{state}_{s['reset']}")
            with y: row["rs_iniziale_pct"]=st.number_input("Residuo secco iniziale (%)",min_value=0.0,max_value=100.0,value=_number(current.get("rs_iniziale_pct")) if current else 0.0,key=f"rsi_{state}_{s['reset']}")
    ...
    if add:
        ...
        elif event_type=="Produzione" and (not row["lotto"] or not row["descrizione"]): st.error("Lotto, codice e descrizione sono obbligatori.")
