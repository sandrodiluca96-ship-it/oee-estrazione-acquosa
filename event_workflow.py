...
def _create_production(machine,lot,shift_id,data_turno,shift,buffer):
    ...
    if machine=="Comber":
        ...
        equivalent=min(final,pure/.40) if final else 0
    ...
    if event_type=="Produzione":
        ...
        if stage=="Apertura lotto":
            ...
            if machine=="EV200":
                ...
                if machine=="Comber":
                    plans=_open_comber_plans()
                    plan_options=[f'{p["piano_id"]} — {p["prodotto"]} — {p["lotto_droga"]} — {p["kg_pianificati"]} kg' for p in plans]
                    old_plan=current.get("piano_id","") if current else ""
                    default_plan=next((x for x in plan_options if x.startswith(str(old_plan)+" — ")),None)
                    chosen=st.selectbox("Campagna pianificata (facoltativa)",["-- Produzione non pianificata --"]+plan_options,index=(["-- Produzione non pianificata --"]+plan_options).index(default_plan) if default_plan else 0,key=f"plan_{state}_{s['reset']}")
                    if chosen!="-- Produzione non pianificata --":
                        row["piano_id"]=chosen.split(" — ")[0]
                        plan=next(p for p in plans if p["piano_id"]==row["piano_id"])
                        st.info(f'Piano: {plan["prodotto"]} · lotto droga {plan["lotto_droga"]} · {plan["estrazioni_pianificate"]} estrazioni · {plan["kg_pianificati"]} kg')
