...
def _comber_extraction_summary(rows):
    ...
    for extraction_rows in numbered.values():
        # La droga può essere riportata anche nella prosecuzione del turno:
        # il massimo per numero estrazione evita il doppio conteggio.
        drug += max([_number(r.get("kg_droga")) for r in extraction_rows]+[0])
    ...
    if event_type=="Produzione":
        ...
        if stage=="Apertura lotto":
            ...
            if machine=="EV200":
                ...
                plan_labels=[f'{p["data_prevista"]} · {p["codice"]} — {p["descrizione"]} · {p["quantita_pianificata"]} kg' for p in plans]
                old_plan=current.get("piano_id","") if current else ""
                old_label=next((label for label,p in zip(plan_labels,plans) if p["piano_id"]==old_plan),None)
                selected=st.selectbox("Riga di pianificazione",["Produzione non pianificata"]+plan_labels,index=(["Produzione non pianificata"]+plan_labels).index(old_label) if old_label else 0,key=f"mixplan_{state}_{s['reset']}")
                if selected!="Produzione non pianificata":
                    plan=plans[plan_labels.index(selected)]
                    code,description=plan["codice"],plan["descrizione"]
                    row["piano_id"]=plan["piano_id"]
                    row["quantita_pianificata"]=_number(plan["quantita_pianificata"])
                    st.success(f"Piano collegato: {code} — {description} · {row['quantita_pianificata']:.1f} kg")
                else:
        ...
        if machine=="Comber":
            ...
            inherited_drug=max([_number(r.get("kg_droga")) for r in extraction_rows]+[0])
            drug_default=_number(current.get("kg_droga")) if current and _number(current.get("kg_droga"))>0 else inherited_drug
        ...
        if st.button("Salva turno",type="primary",disabled=not(valid and confirm),key=f"save_{state}"):
            ...
            for r in buffer:
                if r.get("tipo_evento")=="Produzione" and r.get("tipo_produzione")=="Chiusura lotto":
                    ...
                    # fonte evita di duplicare durata, ore-uomo e quantità.
                    prod=_create_production(machine,r["lotto"],shift_id,day,shift,[])
