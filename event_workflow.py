...
def _open_mescole_plans():
    ...
    return [p for p in plans.to_dict("records") if produced.get(p["piano_id"],0)<_number(p["quantita_pianificata"])]
...
def render_machine_workflow(machine):
    ...
    st.divider(); st.markdown(f"### Modifica eventi salvati — {machine}")
    ...
    if recent.empty:
        ...
        st.caption("Modifica direttamente i dati nella tabella e conferma il salvataggio. ID, turno e macchina sono protetti.")
        ...
        if st.button("Salva correzioni eventi",disabled=not confirm_edit,key=f"save_saved_{state}"):
            ...
            if not valid_all: st.error(error)
                ...
                productions=_read(PRODUZIONI_FILE,COL_PRODUZIONI)
                affected_lots=[x for x in edited["lotto"].unique() if str(x).strip()]
                for lot in affected_lots:
                    productions=productions[~((productions["macchina"]==machine)&(productions["lotto"]==lot))]
                    lot_events=updated[(updated["macchina"]==machine)&(updated["lotto"]==lot)&(updated["tipo_evento"]=="Produzione")]
                    eligible=lot_events[
                    ...
                    if not eligible.empty:
                        ...
                        prod=_create_production(machine,lot,last["id_turno"],last["data_turno"],str(last["turno"]),[])
                        productions=pd.concat([productions,pd.DataFrame([prod])],ignore_index=True)
                _save(productions,PRODUZIONI_FILE,COL_PRODUZIONI)
                turns=_read(TURNI_FILE,COL_TURNI)
                for shift_id in edited["id_turno"].unique():
                    shift_events=updated[updated["id_turno"]==shift_id]
                    if shift_events.empty: continue
                    prod_h=sum(_number(v) for v in shift_events[shift_events["tipo_evento"]=="Produzione"]["durata_h"])
                    idx=turns["id_turno"]==shift_id
                    if idx.any():
                        turns.loc[idx,"ore_produzione"]=prod_h
                        turns.loc[idx,"ore_fermo"]=max(8-prod_h,0)
                _save(turns,TURNI_FILE,COL_TURNI)
