...
from i18n import machine_name, translate
...
class AnalyticsTests(unittest.TestCase):
 def test_interface_translation_keeps_internal_machine_ids_separate(self):
    self.assertEqual(machine_name("Comber","Italiano"),"Estrazione acquosa")
    self.assertEqual(machine_name("Spray Dryer","Italiano"),"Essiccazione Spray Dryer")
    self.assertEqual(machine_name("Comber","English"),"Aqueous Extraction")
    self.assertEqual(translate("Pianificazione Comber","Italiano"),"Pianificazione Estrazione acquosa")
    self.assertEqual(translate("Pianificazione Comber","English"),"Aqueous Extraction Planning")
    self.assertEqual(translate("Essiccazione Spray Dryer","Italiano"),"Essiccazione Spray Dryer")
    self.assertEqual(translate("Production (kg) by line","Italiano"),"Produzione (kg) per linea")
 ...
 def test_plan_is_completed_when_all_planned_extractions_are_complete(self):
    plans=pd.DataFrame([{
        "piano_id":"P37-F","settimana":"37","data_inizio":"2026-09-08","data_fine":"2026-09-09",
        "ora_inizio":"06:00","ora_fine":"22:00","prodotto":"FINOCCHIO","lotto_droga":"",
        "estrazioni_pianificate":10,"kg_per_estrazione":199.1,"kg_pianificati":1991,"impianto":"Comber","caricato_il":"",
    }])
    events=[]
    for extraction in range(1,11):
        events.append({
            **_event(str(extraction),"2026-09-08","Produzione",1,"Finocchio","M26/0137",extraction,199,"Completata"),
            "tipo_produzione":"Prosecuzione lotto","stato_lotto":"In corso",
        })
    progress,_,_=comber_planning_view(plans,pd.DataFrame(events),plans,datetime(2026,9,10,12))
    row=progress.iloc[0]
    self.assertEqual(row["stato"],"COMPLETATO")
    self.assertEqual(row["kg_effettivi"],1990)
    self.assertEqual(row["kg_residui"],0)
    self.assertEqual(row["scostamento_kg"],-1)
